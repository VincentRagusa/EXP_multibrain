...
int RouletteOptimizer::selectParent(std::vector<double>& scores, double maxScore, int popSize){
...
void RouletteOptimizer::optimize(std::vector<std::shared_ptr<Organism>>& population) {
	...
	for (int i = 0; i < popSize; i++) {
		if (numberParents == 1) {
			auto parent = population[selectParent(remappedScores, remappedScoresMax, popSize)];
			population.push_back(parent->makeMutatedOffspringFrom(parent)); // add to population
		}
		else {
			parents.clear();
			do {
				parents.push_back(population[selectParent(remappedScores, remappedScoresMax, popSize)]); // select from culled
		...
		for (auto org : population) {
			org->dataMap.set("roulette_numOffspring", org->offspringCount);
		}
	}
